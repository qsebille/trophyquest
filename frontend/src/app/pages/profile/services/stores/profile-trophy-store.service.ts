...
export class ProfileTrophyStoreService {
  private readonly profileApiService = inject(ProfileApiService);
  private readonly notificationService = inject(NotificationService);
  private readonly pageSize = 20;
  private readonly pagination = signal<Pagination<ProfileTrophyItem> | null>(null);
  ...
  private searchPage(playerId: string, pageNumber: number = this.currentPageNumber()): Observable<Pagination<ProfileTrophyItem>> {
    this._isLoading.set(true);
    this._isError.set(false);

    return this.profileApiService.searchTrophies(playerId, pageNumber, this.pageSize)
      .pipe(
        map(pagination => {
          const content = pageNumber === 0 ? pagination.content : [...this.trophies(), ...pagination.content];
          return {...pagination, content} as Pagination<ProfileTrophyItem>;
        }),
        catchError(err => {
