...
export class IgdbMappingStoreService {
  ...
  search(pageNumber: number = this.pageNumber): void {
    if (this._status() === LoadingStatus.LOADING ||
      this._status() === LoadingStatus.FULLY_LOADED) {
      return;
    }
    ...
    this.igdbCandidateApiService.searchPending(pageNumber, this.pageSize)
      .pipe(
        tap(searchResult => {
          const mappings = [...this._mappingList(), ...searchResult.content];
          const loadingStatus = mappings.length < searchResult.total ? LoadingStatus.PARTIALLY_LOADED : LoadingStatus.FULLY_LOADED;

          this._mappingList.set(mappings);
